...
def update_record():
    ...
    cohort_name = ui.lineEdit_6.text()
    cohort_number = ui.lineEdit_8.text()
    ...
    if len(results) > 0:
        ...
        ui.label_6.setText("A study group already exists with the given cohort name and number")

    else:
            # Check if a record with the given group ID exists
            sql = "SELECT * FROM studyGroups WHERE groupId=%s"
            params = (groupid,)
            cursor.execute(sql, params)
            results = cursor.fetchall()
            ...
            if len(results) == 0:
                ui.label_6.setText("No study group found with the given group ID")
            else:
                ...
                if len(results) > 0:
                    ...
                    if len(results) > 0:
                        ...
                        ui.label_6.setText("No study group found with the given cohort name and group ID")
...
def delete_record():
    ...
    if len(results) == 0:
        # Display a message saying that the record does not exist
        ui.label_6.setText("No study group found with the given cohort name and number")
    else:
        # Get the groupid of the study group
        groupid = results[0][0]
        
        # Delete all courses for the given groupid
        sql = "DELETE FROM Courses WHERE groupId=%s"
        params = (groupid,)
        cursor.execute(sql, params)
        mydb.commit()
        
        # Delete all students with the given groupid
        sql = "DELETE FROM Students WHERE groupId=%s"
        params = (groupid,)
        cursor.execute(sql, params)
        mydb.commit()
        
        # Delete the study group with the given cohort name and number
        sql = "DELETE FROM studyGroups WHERE cohortName=%s AND cohortNumber=%s"
        params = (cohort_name, cohort_number)
        cursor.execute(sql, params)
        mydb.commit()
...
if __name__ == "__main__":
    ...
    #UPDATE BUTTON 
    ui.pushButton_4.clicked.connect(update_record)
    ui.pushButton_5.clicked.connect(delete_record)
    ####################3################################################################
#THE CANCEL BUTTON SHOULD RETURN TO PREVIOUS PAGE PUSHBUTTON_6
   # ui.pushButton_6.clicked.connect(MainWindow.close)
    MainWindow.show()
    sys.exit(app.exec_())
